...
base_dir = "results_virulence"  # Ajusta al nombre de tu carpeta de virulencia
...
if not os.path.exists(base_dir):
    ...
    for isolate_name in os.listdir(base_dir):
        ...
        if os.path.isdir(isolate_path):
            # Buscamos archivos .json dentro de la carpeta del aislado
                ...
                try:
                    ...
                    if hits:
                        for hit in hits.values():
                            # Copiamos todos los datos del hit (grade, identity, coverage, etc.)
                            row = hit.copy()
                            row['Aislado'] = isolate_name
                            
                            # Aplanamos campos que suelen ser listas o diccionarios complejos
                        ...
                        # Si no hay factores de virulencia detectados
...
if final_data:
    ...
    # Reordenamos para que 'Aislado' y 'name' (nombre del gen) aparezcan primero
    ...
    # Generar Matriz Binaria de Virulencia para el TFM

    ...
    if not json_files:
        ...
        for j_file in json_files:
            ...
            try:
                ...
                # Buscar los hits en 'seq_regions'
                hits = data.get('seq_regions', {})
                
                if hits:
                    for hit in hits.values():
                        # Copiar todos los datos del hit (todas las columnas)
                        row = hit.copy()
                        row['Aislado'] = isolate_name
                        
                        # Convertir listas/diccionarios a texto para Excel
                        for key in row.keys():
                    ...
                    # Registrar como negativo si no hay hits
                    final_data.append({"Aislado": isolate_name, "name": "Negativo", "identity": 0})
                        
            except Exception as e:
                print(f"Error procesando {j_file}: {e}")

# 3. Crear los archivos de salida
if final_data:
    df = pd.DataFrame(final_data)
    
    # Poner 'Aislado' y 'name' (el plásmido) al principio
    ...
    # Pivotar usando el nombre del plásmido
